...
class wpPubMedRefList
{
    ...
    */
    public function wp_pubmed_reflist($key, $limit=50, $style, $wrap, $linktext, $subset = 1, $showlink)
    {
        $debug = false;
        $html =  $msg = '';
        $query_results;
    ...
    */
    public function reflist_query($key, $limit)
    {
        $query = $this->query;
        if ($query == "($key)") {
            return "please enter a query for $key in the admin panel<br>";
        }
    ...
    public function add_extras($key)
    {
    ...
    */
    public function build_recursive_query($query)
    {
    ...
    public function getQueryFromOptions($key)
    {
        $key = trim($key, "\n ()");
        if (
