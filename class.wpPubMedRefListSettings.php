...
class wpPubMedRefListSettings
{
    ...
    public function query_form()
    {
        ...
        if ($faclist) {
            ...
            foreach ($faclist as $name) {
                ...
                if (isset($facprops[$name]['extras'])) {
                    $extras = $facprops[$name]['extras'];
                }
                echo "<td><textarea id='wp_pubmed_reflist_qlist' ".
    ...
    */
    public function faclist_form()
    {
        $faclist = array();
        $facprops = @$this->options['facprops'];
        if ($this->options && isset($this->options['faclist'])) {
            $faclist = $this->options['faclist'];
        }
    ...
    public function style_props_form()
    {
        # get the previous value returns false if empty
        ...
        if ($stylelist) {
            ...
            foreach ($stylelist as $name) {
                ...
                $template = '';
                if (isset($styleprops[$name]['format'])) {
                    $template = $styleprops[$name]['format'];
                }

                # max number of authors in author list
                $authlimit = '';
                if (isset($styleprops[$name]['authlimit'])) {
                    $authlimit = $styleprops[$name]['authlimit'];
                }
                # number of authors to show in author list if > $authlimit
                $authshow = '';
                if (isset($styleprops[$name]['authshow'])) {
                    $authshow = $styleprops[$name]['authshow'];
                }
    ...
    We use hidden input fields to make sure we keep all the old values
    */
    public function styles_form()
    {
        $this->set_default_styles();
        $styleProps = $this->formats['styleprops'];
        if ($this->formats && isset($this->formats['stylelist'])) {
            $styleList = $this->formats['stylelist'];
        }
    ...
    */
    ...
    public function styles_ital_form()
    {
        $itals = '';
        if ($this->formats && isset($this->formats['itals'])) {
            $itals = $this->formats['itals'];
        }
    ...
    public function styles_bold_form()
    {
        $itals = '';
        if ($this->formats && isset($this->formats['bold'])) {
            $itals = $this->formats['bold'];
        }
        echo "<textarea id='wp_pubmed_reflist_stylelist' ".
    ...
    TODO!
    */
    public function pubmed_queries_validate($text)
    {
        #	echo "is this callback used? $text";
        return $text;
    }
    public function pubmed_faclist_validate($text)
    {
        return $text;
    }
    public function pubmed_formats_validate($text)
    {
        return $text;
    }
}
